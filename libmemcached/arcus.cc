...
ALREADY_INITIATED:
  pthread_mutex_unlock(&lock_arcus);
  return ARCUS_ALREADY_INITIATED;
}
...
static inline arcus_return_t do_arcus_zk_connect(memcached_st *mc)
{
  ...
  {
    ZOO_LOG_ERROR(("zookeeper_init() failed, reason=%s, zookeeper=%s", strerror(errno), arcus->zk.ensemble_list));
    goto ERROR;
  }
...
static inline arcus_return_t do_arcus_zk_close(memcached_st *mc)
{
  ...
  if (arcus->zk.handle)
  {
    ...
    if (zrc != ZOK)
    {
      ZOO_LOG_WARN(("zookeeper_close() failed, reason=%s, ensemble=%s", zerror(zrc), arcus->zk.ensemble_list));
      goto ERROR;
    }
  }
                                                          ...
                                                          watcher_fn watcher)
{
  ...
  {
    ZOO_LOG_ERROR(("zoo_wget_children() failed, reason=%s, zookeeper=%s", zerror(zkrc), arcus->zk.ensemble_list));
  }
}
